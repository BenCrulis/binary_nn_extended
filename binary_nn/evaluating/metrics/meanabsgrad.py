...
def computes_mean_abs_grad(mod: nn.Module, device=None):
    p = [p.grad.flatten() for p in mod.parameters() if p.grad is not None]
    if len(p) == 0:
        return torch.tensor([0.0], device=device)[0]
    p = torch.cat(p)
    return torch.abs(p).mean()


class MeanAbsGrad(Metric):
    ...
    def update(self, *args, **kwargs) -> None:
        norms = torch.stack([computes_mean_abs_grad(m, device=self.sum.device)
