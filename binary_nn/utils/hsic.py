...
def estimate_hsic_zy_objective(z, y, z_kernel, y_kernel, gamma=2.0, mode='biased'):
    """
    ...
    return estimate_hsic(z_matrix, z_matrix, mode) - gamma * estimate_hsic(y_matrix, z_matrix, mode)
...
class GaussianKernel(Kernel):
    """
    Gaussian kernel: k(a_i, a_j) = exp(-||a_i - a_j||^2 / (2 sigma^2))
    """
    ...
    def compute(self, batch):
        ...
        """
        return torch.exp(-(compute_pdist_matrix(batch, p=2.0)) ** 2 / (2.0 * self.sigma ** 2))
