"""Adapted from https://github.com/zrsmithson/Stanford-dogs/blob/master/data/stanford_dogs_data.py"""
...
class StanfordDogs(Dataset):
    ...
    def __init__(self,
        ...
        self.annotations_folder = self.root / 'Annotation'
        self._breeds = list(self.images_folder.iterdir())

        if self.cropped:
            self._breed_annotations = [[(annotation, box, idx)
                                        for box in self.get_boxes((self.annotations_folder / annotation))]
    ...
    def download(self):
        ...
        if (self.root / 'Images').exists() and (self.root / 'Annotation').exists():
            if len(list((self.root / 'Images').iterdir())) == len(list((self.root / 'Annotation').iterdir())) == 120:
    ...
    def load_split(self):
        if self.train:
            split = scipy.io.loadmat(self.root / 'train_list.mat')['annotation_list']
            labels = scipy.io.loadmat(self.root / 'train_list.mat')['labels']
        else:
            split = scipy.io.loadmat(self.root / 'test_list.mat')['annotation_list']
            labels = scipy.io.loadmat(self.root / 'test_list.mat')['labels']
...
def load_dogs(config, augment=False):
    from binary_nn.datasets.commons import get_train_transforms, get_test_transforms
    dogs_path = Path(config["stanforddogs"]["path"]).absolute()
    train_transforms = get_train_transforms(augment)
    test_transforms = get_test_transforms()

    ds = StanfordDogs(dogs_path, train=True, transform=train_transforms, download=True)
    test_ds = StanfordDogs(dogs_path, train=False, transform=test_transforms)
    ds.stats()
    n_classes = 120
...
if __name__ == '__main__':
    ds = StanfordDogs("C:\\data\\datasets\\StanfordDogs", download=True)
    # ds = StanfordDogs("C:\\data\\datasets\\TestDogs", download=True)
    ds.stats()
    im = ds[0]
    pass
